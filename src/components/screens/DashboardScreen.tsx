...
function IconBtn({ onClick, children, ariaLabel }: { onClick?: () => void; children: React.ReactNode; ariaLabel: string }) {
  return (
    <button className="icon-btn" onClick={onClick} aria-label={ariaLabel}>{children}</button>
  );
}
...
export function DashboardScreen({ data, onOpenEntry, onNavigate }: Props) {
  ...
  const proximos = [...fisicoEntries, ...digitalEntries]
    ...
    <div>
      ...
      <header className="app-header">
        ...
        <div style={{ flex: 1 }}>
          ...
          <h1>Catavento · Abril</h1>
        </div>
        <IconBtn ariaLabel="Notificações"><IconBell size={20} /></IconBtn>
      </header>
        ...
        >
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <div className="balance">
              ...
              {fmtAbs(saldoPrevisto)}
            </div>
            <div style={{ display: 'flex', gap: 22, marginTop: 18, position: 'relative', zIndex: 1 }}>
              <div>
                <div style={{ fontSize: 11, opacity: 0.7, fontWeight: 500, color: 'var(--cv-bg)' }}>Receitas previstas</div>
                <div style={{ fontSize: 15, fontWeight: 700, marginTop: 2, fontVariantNumeric: 'tabular-nums', letterSpacing: '-0.005em', color: '#6FD49B' }}>
                  + {fmtAbs(totalPrevisto)}
                </div>
              </div>
              <div>
                <div style={{ fontSize: 11, opacity: 0.7, fontWeight: 500, color: 'var(--cv-bg)' }}>Despesas do mês</div>
                <div style={{ fontSize: 15, fontWeight: 700, marginTop: 2, fontVariantNumeric: 'tabular-nums', letterSpacing: '-0.005em', color: '#FFA9A6' }}>
                  − {fmtAbs(gastosFisicos + gastosDigitais)}
                </div>
              </div>
            </div>
          </div>
      ...
      <div style={{ padding: '16px 20px 0' }}>
        ...
        >
          {[
            { label: 'A receber', tone: 'up', icon: <IconArrowDown size={16}/>, value: fmtBRL(aReceber), sub: `${receitas.filter(r => r.status === 'Previsto').length} lançamentos` },
            { label: 'Pago', tone: 'default', icon: <IconCheck size={16}/>, value: fmtBRL(valorPago), sub: 'quitado este mês' },
            { label: 'Pendente', tone: 'warn', icon: <IconClock size={16}/>, value: fmtBRL(valorPendente), sub: `${allEntries.filter(x => x.status === 'Pendente').length} lançamentos` },
            { label: 'Atrasado', tone: 'down', icon: <IconAlert size={16}/>, value: fmtBRL(valorAtrasado), sub: `${allEntries.filter(x => x.status === 'Atrasado').length} lançamento` },
          ].map((s) => (
            <motion.div key={s.label} className="stat" variants={listItem}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                <div className="stat-label">{s.label}</div>
                <div className="stat-badge" style={{ background: statBg[s.tone].bg, color: statBg[s.tone].fg }}>
                  {s.icon}
                </div>
              </div>
      ...
      <div className="section-label">
        <h3>Físico × Digital</h3>
      </div>
      <div style={{ padding: '0 20px' }}>
        <motion.div className="card" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}>
          <div className="donut-wrap">
            <Donut slices={[
              { color: '#26325B', value: gastosFisicos },
              { color: '#E9BE2B', value: gastosDigitais },
            ]} />
            <div className="donut-legend">
              <div className="lg-item">
                <div className="lg-swatch" style={{ background: '#26325B' }} />
                <div className="lg-text">Físico</div>
                <div className="lg-val">{fmtBRL(gastosFisicos)}</div>
              </div>
              <div className="lg-item">
                <div className="lg-swatch" style={{ background: '#E9BE2B' }} />
                <div className="lg-text">Digital</div>
                <div className="lg-val">{fmtBRL(gastosDigitais)}</div>
              </div>
              <div style={{ height: 1, background: 'var(--cv-gray-200)', margin: '2px 0' }} />
              <div className="lg-item">
                <div className="lg-swatch" style={{ background: 'transparent' }} />
                <div className="lg-text" style={{ color: 'var(--cv-gray-600)' }}>Total</div>
                <div className="lg-val">{fmtBRL(gastosFisicos + gastosDigitais)}</div>
              </div>
            </div>
          </div>
        </motion.div>
      ...
      >
        {proximos.length === 0 && (
          <div className="card muted" style={{ textAlign: 'center', padding: 24, color: 'var(--cv-gray-600)', fontSize: 14 }}>
            Nenhum vencimento pendente. Ótimo!
          </div>
        )}
        {proximos.map(e => (
          <motion.div
          ...
          >
            ...
            <div className="row-grow">
              <div className="row-top">
                ...
                <div className="row-amount">{fmtBRL(e.value)}</div>
              </div>
              <div className="row-bot">
                <div className="row-meta">Vence {e.due} · {e.cat}</div>
      ...
      {/* Receitas preview */}
      <div className="section-label">
        <h3>Receitas · Abril</h3>
        <button className="see-all" onClick={() => onNavigate('receitas')}>Ver tudo →</button>
      </div>
      <motion.div
        style={{ padding: '0 20px 0', display: 'flex', flexDirection: 'column', gap: 10 }}
        initial="hidden"
        animate="show"
