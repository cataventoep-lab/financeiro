...
export function EntryListScreen({ kind, entries, showValues, onToggleValues, onFilter, onOpenEntry }: Props) {
  ...
  const maskVal = (v: string) => showValues ? v : 'R$ ●●●●';

  const summaryBg = kind === 'fisico'
    ? 'var(--cv-blue)'
    : 'linear-gradient(135deg, #F1C846, #E9BE2B)';
    ...
    <div>
      ...
      <header className="app-header">
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="icon-btn" onClick={onToggleValues} aria-label="Mostrar/ocultar valores">
      ...
      <div style={{ padding: '0 20px' }}>
        <motion.div
          className="card"
          style={{ background: summaryBg, color: summaryColor, borderRadius: 'var(--r-2xl)' }}
          initial={{ opacity: 0, y: 8 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.25 }}
        >
          <div style={{ fontSize: 11, opacity: 0.75, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
            Total do mês
          </div>
          <div style={{ fontSize: 32, fontWeight: 800, marginTop: 6, lineHeight: 1, fontVariantNumeric: 'tabular-nums', letterSpacing: '-0.02em' }}>
            {maskVal(fmtBRL(total))}
          </div>
      ...
      >
        {visible.length === 0 && (
          <div className="card muted" style={{ textAlign: 'center', padding: 32, color: 'var(--cv-gray-600)', fontSize: 14 }}>
            ...
            {q && <span>Tente outro termo de busca.</span>}
          </div>
        )}
        {visible.map(e => (
          <motion.div
          ...
          >
            ...
            <div className="row-grow">
              <div className="row-top">
                ...
                <div className="row-amount">{maskVal(fmtBRL(e.value))}</div>
              </div>
              <div className="row-bot">
                <div className="row-meta">
                  {e.status === 'Pago' ? `Pago ${e.paid}` : `Vence ${e.due}`} · {e.cat}
                </div>
